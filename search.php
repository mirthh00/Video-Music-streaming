    ...
    <div id="preloader"></div>
        ...
        <nav class="flex-div">
            ...
            <form method="POST" action="search.php">
                <input type="text" class="search" placeholder="Search Movies" name="search">
        ...
        if(isset($_GET["search_query"]))
        {
            ...
            $movies="Music";
            $search2=$_GET["search_query"];
            $sql3="SELECT * FROM videos WHERE  videotitle LIKE '%$search2%' or uploader LIKE '%$search2%' ORDER BY id DESC;";
            $result3=mysqli_query($conn,$sql3);
            $searchQueryResult2=mysqli_num_rows($result3);
            ...
            if($searchQueryResult2 != 0)
            {
                    ...
                    while($row3=mysqli_fetch_assoc($result3)){
                        include "includes/accountdatabase.inc.php";
                        $userid=$_SESSION["fullname"];
                        $sql4="SELECT * FROM unlockedaccounts WHERE  userfullname=? AND accountname=?;";
                        $stmt=mysqli_stmt_init($conn);
                        ...
                        mysqli_stmt_bind_param($stmt,"ss",$userid,$row3["uploader"]);
                        mysqli_stmt_execute($stmt);
                        $result4=mysqli_stmt_get_result($stmt);
                        mysqli_stmt_close($stmt);
                        $row4=mysqli_fetch_assoc($result4);
                        $rowsnumber2=mysqli_num_rows($result4);
                        ...
                        if($rowsnumber2>0)
                        {
                            ...
                            <input type="image" src="content/videos/thumbnails/'.$row3["uploader"].'/'.$row3["thumbnail"].'" class="thumbnail" name="thumbnail">
                            </form>
                            <p class="videodescription">'.$row3["videotitle"].'
                            </p>
                            <div class="videodetails flex-div">
                                <a href="account.php?ID='.$row4["id"].'"><img src="profilepictures/'.$row4["profileimage"].'" class="accounticon"></a>
                                <div class="videoinfo">
                                    <p class="videoviews">'.$row3["views"].' views | '.$currenttime.'</p>
                                </div>
                            </div>
                        </div>
                            ';
                        ...
                        $row5=mysqli_fetch_assoc($result5);
                            ...
                            echo '
                            <div class="video">
                            <form action="subscribe.php?ID='.$row5["id"].'" method="POST">
                            <input type="hidden" value="'.$row3["uniqueid"].'" name="uniqueid">
                            <input type="image" src="content/videos/thumbnails/'.$row3["uploader"].'/'.$row3["thumbnail"].'" class="thumbnail" name="thumbnail">
                            </form>
                            <p class="videodescription">'.$row3["videotitle"].'
                            ...
                            <div class="videodetails flex-div">
                                <a href="subscribe.php?ID='.$row5["id"].'"><img src="profilepictures/'.$row5["profileimage"].'" class="accounticon"></a>
                                <div class="videoinfo">
                                    <p class="videoviews">'.$row3["views"].' views | '.$currenttime.'</p>
                                </div>
                            </div>
