...
if(isset($_GET["c"])){
    ...
    $sql6="SELECT * FROM music WHERE  song=?;";
...
$sql6="SELECT * FROM music WHERE  uploader=? ORDER BY id DESC ;";
$stmt=mysqli_stmt_init($conn);
...
if(isset($_POST["action"])){
    ...
    $sql9="UPDATE music SET streams=streams+1 WHERE song=?;";
    $stmt=mysqli_stmt_init($conn);
    ...
    $sql20="SELECT * FROM musicanalytics WHERE song=? AND day=?;";
    $stmt=mysqli_stmt_init($conn);
    ...
    $sql23="SELECT * FROM music WHERE song=?;";
    $stmt=mysqli_stmt_init($conn);
    ...
    if($rownum20==0){
        $streams=1;
        $sql21="INSERT INTO musicanalytics(song,songtitle,songcover,streams,totalstreams,day,uploadedtime) VALUES (?,?,?,?,?,?,?);";
        $stmt=mysqli_stmt_init($conn);
        ...
        mysqli_stmt_bind_param($stmt, "sssssss",$videoid,$row23["songtitle"],$row23["song"],$streams,$totalstreams,$day,$uptime);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
    }
    else{
        $sql22="UPDATE musicanalytics SET streams=streams+1 WHERE song='$videoid' AND day='$day';";
        mysqli_query($conn,$sql22);
        $sql22="UPDATE musicanalytics SET totalstreams='$totalstreams' WHERE song='$videoid' AND day='$day';";
        mysqli_query($conn,$sql22);
    }

    $country=$_SESSION["country"];

    $sql24="SELECT * FROM musicstats WHERE song=? AND country=?;";
    $stmt=mysqli_stmt_init($conn);
    ...
    if($rownum24==0){
        $streams=1;
        $sql21="INSERT INTO musicstats(song,songtitle,songcover,streams,totalstreams,uploaderprofile,country,uploadedtime) VALUES (?,?,?,?,?,?,?,?);";
        $stmt=mysqli_stmt_init($conn);
        ...
        mysqli_stmt_bind_param($stmt, "ssssssss",$videoid,$row23["songtitle"],$row23["song"],$streams,$totalstreams,$row23["uploaderprofile"],$country,$row23["uploadedtime"]);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
    }
    else{
        $sql22="UPDATE musicstats SET streams=streams+1 WHERE song='$videoid' AND country='$country';";
        mysqli_query($conn,$sql22);
        $sql22="UPDATE musicstats SET totalstreams='$totalstreams' WHERE song='$videoid' AND country='$country';";
        mysqli_query($conn,$sql22);
    }
}
    ...
    <div id="preloader"></div>
        ...
        <nav class="flex-div">
            ...
            <div class="nav-middle flex-div" >
                ...
                <input type="text" class="search" placeholder="Search Music" id="search_query">
                
                <input type="text" class="search1" placeholder="Search Music" id="search_query1">
            ...
            <div class="nav-right flex-div">
                ...
                <?php
                        if(isset($_SESSION['username']))
                        {
                            ...
                            <li><a  href='includes/theme.inc.php?t=dark&l=music'><i id='dark' class='fas fa-moon'></i>Dark Theme</a></li>
                             <br>
                             <li><a  href='includes/theme.inc.php?t=light&l=music'><i id='light' class='fas fa-sun'></i>Light Theme</a></li>
                             <br>
        ...
        <div class="sidebar">
            ...
            <li><a href="home.php" ><i class="fas fa-film"></i>Series</a></li>
                ...
                <li><a href="#" id="selected"><i class="fas fa-music"></i>Music</a></li>
            ...
            <div class="musicsection">
                ...
                <div class="contentcontainer">
                    ...
                    if(isset($_GET["song"])){
                        ...
                        $sql12="SELECT * FROM music WHERE  uploader LIKE CONCAT('%',?,'%') OR songtitle LIKE CONCAT('%',?,'%') ORDER BY id DESC ;";
                        $stmt=mysqli_stmt_init($conn);
                        ...
                        if($rownum12>0)
                        {
                            while ($row12=mysqli_fetch_assoc($result12)) {
                                ...
                                $songstreams = $row12["streams"];
                                $songstreams = currencyFormat($songstreams);
                                include 'includes/accountdatabase.inc.php';
                        ...
                        <div class="video">
                            ...
                            <div class="c">
                                ...
                                <div class="videodetails">
                                    ...
                                    <div class="videoinfo">
                                        <p class="videoviews lowfont"><?php echo "$songstreams"; ?> Streams &bullet; <?php echo "$uploadedtime"; ?></p>
                                    </div>
                                </div>
                            </div>
                                <?php 
                                       include 'includes/contentdatabase.inc.php';
                                       $sql14="SELECT * FROM likedmusic  WHERE  songid=? and username=?;";
                                       $stmt=mysqli_stmt_init($conn);
        ...
        <li>
            <div class="di">
                <a href="#"><i id="chosen" class="fas fa-music"></i></a>
                <p id="chosen">Music</p>
            </div>
...
const songs=[
    <?php
        if (isset($_GET["song"])){
            ...
            $sql11="SELECT * FROM music WHERE  uploader LIKE CONCAT('%',?,'%') OR songtitle LIKE CONCAT('%',?,'%') ORDER BY id DESC ;";
            $stmt=mysqli_stmt_init($conn);
            ...
            $sql7="SELECT * FROM music WHERE uploader=? ORDER BY id DESC;";
            $stmt=mysqli_stmt_init($conn);
...
Array.from(document.getElementsByClassName('songplay2')).forEach((element)=>{
    element.addEventListener('click', (e)=>{
        ...
        poster_master_player.src = `music/covers/${index}.jpg`;
        masterPlay.classList.remove('fa-play');
        masterPlay.classList.add('fa-pause');
        poster_master_player.style.animation='spin 1s linear infinite';
        music.play();
