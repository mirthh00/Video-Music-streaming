...
<?php
    ...
    if(isset($_POST["sn"])){
        $saname=mysqli_real_escape_string($conn,$_POST["sn"]);
    ...
    if(isset($_POST["ky"])){
        $key=mysqli_real_escape_string($conn,$_POST["ky"]);
    }
    else{
        $key ="none";
    }
    ...
    if($saname != "none" && $key == "keyin"){
    header("location: ../watchcontent.php?watch=$uniqueid&a=$accountname&y=$uniqueid"); 
    }
    else{
